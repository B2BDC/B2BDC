...
for i = 1:n_units
   ...
   switch modelM{1}
      case 'quadratic'
         if length(modelM) == 2
            quadCoef = modelM{2};
            unitModel = generateModel(quadCoef,varList);
         elseif length(modelM) == 3
            Xdata = modelM{2};
            Ydata = modelM{3};
            unitModel = generateModelbyFit(Xdata,Ydata,varList);
         end
      case 'rational quadratic'
         if length(modelM) == 4
            ...
            matrixDen = modelM{3};
            kVal = modelM{4};
            unitModel = generateModel(matrixNum,matrixDen,varList, kVal);
         elseif length(modelM) == 3
            if issymmetric(modelM{2})
               unitModel = generateModel(matrixNum,matrixDen,varList, kVal);
            else
               opt = generateOpt;
               Xdata = modelM{2};
               Ydata = modelM{3};
               unitModel = generateModelbyFit(Xdata,Ydata,varList,'rq',opt,kVal);
            end
         else
            opt = generateOpt;
            Xdata = modelM{2};
            Ydata = modelM{3};
            unitModel = generateModelbyFit(Xdata,Ydata,varList,'rq',opt);
         end
   end
