function Xvals = collectSamples(obj,N,xStart,opt)
...
if isempty(Q)
   ...
   for i1=1:n2
      ...
      V(zPDir) = -abs(V(zPDir));
      V(zNDir) = abs(V(zNDir));
      tmpx = B2BDC.B2Bvariables.VariableList.q2sample(Mgd,idx,H,xInit,V');
%       tmpx = B2BDC.B2Bvariables.VariableList.quadSample(Mgd,H,xInit,V');
      if ~obj.isFeasiblePoint(tmpx)
         keyboard
      end
      if mod(i1,nstep) == 0
         Xvals(ic,:) = tmpx';
