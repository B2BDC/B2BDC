function [yin_result,s,xopt] = relCMfminconNN(obj,disflag,b2bopt)
...
if isempty(obj.FeasiblePoint)
   ...
   for j = 1:nStart
      [x0,yin_result,exitflag,~,ilam] = fmincon(@funxmin,[0;xStart(j,:)'],...
         A1,B1,Aeq,Beq,[-Inf;xLB],[1;xUB],@neq,opt);
...
if ~isempty(A1)
   ...
   function [c,ceq,g,geq] = neq(x)
      ...
      c(2:2:end) = -c(1:2:end);
      g(:,2:2:end) = -g(:,1:2:end);
      c(1:2:end) = c(1:2:end)-(1-x(1))*UB-x(1)*OB;
      c(2:2:end) = c(2:2:end)+(1-x(1))*LB+x(1)*OB;
      g(1,1:2:end) = UB-OB;
      g(1,2:2:end) = OB-LB;
      ceq = [];
      geq = [];
