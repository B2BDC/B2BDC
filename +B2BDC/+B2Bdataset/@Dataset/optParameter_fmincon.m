<<<<<<< Updated upstream
function xopt = optParameter_fmincon(obj,id,flag,Nflag,Hflag,w,b2bopt)
% function XOPT = OPTPARAMETER_FMINCON(OBJ,ID,FLAG,NFLAG,HFLAG,W,B2BOPT) 
...
UB = xbd(:,2);
if Nflag
   opt = optimoptions('fmincon','Display','none','GradObj','on','MaxFunctionEvaluations',3000,...
...
=======
function xopt = optParameter_fmincon(obj,id,Nflag,Hflag,w,b2bopt)
% function XOPT = OPTPARAMETER_FMINCON(OBJ,ID,FLAG,NFLAG,HFLAG,W,B2BOPT) 
% returns the optimal parameter according to the optimization standard
